...
function main() {
	...
	const sizeKB = (
		Buffer.byteLength(JSON.stringify(icons), 'utf8') / 1024
	).toFixed(1);
	console.log(
		`icons.json written: ${Object.keys(icons).length} icons, ${sizeKB} KB${skipped ? ` (${skipped} skipped)` : ''}`
	);
	console.log(`  -> ${out}`);
}
