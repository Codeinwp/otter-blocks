...
/**
 * Markup shipped between the original close button (v1) and the accessible
 * close button. The only difference from the current `save` is the close
 * button, which exposed its label through a `title` attribute only (no
 * `aria-label` or `screen-reader-text`). Without this deprecation, popups saved
 * with that markup fail block validation after the a11y change.
 */
const v2 = {
	...v1,
	save: ({ attributes, className }) => {
			...
			<div { ...blockProps }>
				<div className="otter-popup__modal_wrap">
					...
					<div className="otter-popup__modal_content">
						{ attributes.showClose && (
							<div className="otter-popup__modal_header">
								<button title={ __( 'Close Button', 'otter-blocks' ) } type="button" className="components-button has-icon"><svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true"><path d="M12 13.06l3.712 3.713 1.061-1.06L13.061 12l3.712-3.712-1.06-1.06L12 10.938 8.288 7.227l-1.061 1.06L10.939 12l-3.712 3.712 1.06 1.061L12 13.061z"></path></svg></button>
							</div>
						) }
