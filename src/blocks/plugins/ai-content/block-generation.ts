...
export const ATOMIC_WIND_ATTRIBUTE_HINT = [
	...
	'- Overlay / tint over a background: keep the background (image or color) on the container and give it "relative overflow-hidden"; add an EMPTY atomic-wind/box child (no text, no inner blocks) with "absolute inset-0" and a translucent background ("bg-black/40", or a gradient "bg-gradient-to-t from-black/70 via-black/20 to-transparent"); then give the real content box "relative z-10" so it sits above the overlay. Layering in general: "relative" on the parent, "absolute inset-0" + "z-*" on the layers.',
