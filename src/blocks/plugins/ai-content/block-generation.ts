...
const buildAttributePrompt = (
	...
	return [
		...
		'When a block exposes color attributes (e.g. background, text, or accent colors), set tasteful, harmonious colors so the result looks polished instead of bland. Keep the palette coherent across the whole layout and ensure text stays readable against its background. Leave the remaining styling attributes (sizes, CSS) untouched unless essential.',
