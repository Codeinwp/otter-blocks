...
/**
 * Get the post type and post id.
 *
 * @returns {{postType: string, postId: string|undefined}}
 */
export function getPostTypeAndId() {
	...
	if ( ! postId ) {
		...
		/**
		 * If we are in the FSE, we need to get identify the post ID and post type from the URL.
		 */
		const urlParams = new URLSearchParams( window.location.search );
		postId = urlParams.get( 'postId' ) ?? urlParams.get( 'post' );
		postType = urlParams.get( 'postType' ) ?? 'post';
	}
...
/**
 * This timer assure us that we do not overload the server with requests with cumulative changes.
 */
let timer = null;

/**
 * Trigger the save in Editor.
 */
export function triggerSave() {
	...
	/**
	 * We are changing a special meta field to trigger the save.
	 * The Form Options are saved in the WP Options table when you press Save and not in the block attributes.
	 * Because of this the Editor does not know that the block has changed and it does not trigger the save.
	 * Thus, the Save button is keep its state. Which mean that if you do change any content in the editor, the Save button will not be enabled.
	 *
	 * Below are some small tricks to make sure that the Save button is enabled when you change the Form Options.
	 */

	const { postId, postType } = getPostTypeAndId();
	...
	/**
	 *
	 * A simple trick is to use a dummy meta value that is not used anywhere else.
	 * We set a random string to make sure the value is different every time.
	 *
	 * We use a timer to not overload the server with requests.
	 */
	timer = setTimeout( () => {
		const randomString = Math.random().toString( 36 ).substring( 2, 15 ) + Math.random().toString( 36 ).substring( 2, 15 );
		dispatch( 'core' ).editEntityRecord( 'postType', postType, postId, { meta: { 'themeisle_blocks_mark_to_save': randomString }});
	}, 3000 );
};
