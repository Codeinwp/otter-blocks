...
class Pro {
	...
	public function dashboard_upsell_notice() {
		...
		printf(
			'<div class="notice notice-info is-dismissible">
				...
				<p>
					<b>%1$s</b> %2$s <a target="_blank" href="%3$s">%4$s</a> <a class="notice-dismiss" href="%5$s"><span class="screen-reader-text">%6$s</span></a>
				</p>
			</div>',
			esc_html( $prefix ),
			esc_html( $message ),
	...
	public function dismiss_dashboard_notice() {
		if ( isset( $_GET['otter-dismiss-dashboard-upsell'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( esc_attr( $_GET['_wpnonce'] ), 'dismiss' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$notifications                     = get_option( 'themeisle_blocks_settings_notifications', array() );
